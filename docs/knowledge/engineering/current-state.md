...
- In progress: Final verification and review for `docs/plans/2026-06-30-002-feat-open-gpui-mature-adapter-plan.md`. The adapter crate owns reusable projection, controls, repeatables, menus/actions, inspector, and product fixture gates; `canvas-jellyflow` consumes those helpers and should not regain adapter ownership.
- Blocked: None.
- Next action: Run the final verification contract across main Jellyflow and `repo-ref/open-gpui`, then review the diff for any remaining overclaim around GPUI layout-pass measurement.
