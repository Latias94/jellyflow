...
- Goal: Complete the Open GPUI layout-pass measurement stage while preserving the headless semantic contract and adapter-local widget boundary.
...
- In progress: None for the Open GPUI Layout-Pass Measurement plan. The adapter crate owns reusable projection, controls, repeatables, menus/actions, inspector, measurement conversion, and product fixture gates; `canvas-jellyflow` consumes those helpers and should not regain adapter ownership.
- Blocked: None.
- Next action: Review the committed diff in main Jellyflow and local `repo-ref/open-gpui` main. The next product-stage plan should focus on real interactive authoring polish and visual regression for the Open GPUI adapter, not on expanding egui/Dioxus.
