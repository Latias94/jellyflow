# Architecture Decision Records

This directory uses standalone Jellyflow ADR numbering.

| ADR | Status | Title |
| --- | --- | --- |
| [ADR 0001](0001-jellyflow-headless-node-graph-engine-boundary.md) | Accepted | Jellyflow Headless Node Graph Engine Boundary |
| [ADR 0002](0002-jellyflow-model-policy-boundary.md) | Accepted | Jellyflow Model Policy Boundary |
| [ADR 0003](0003-headless-adapter-testing-and-renderer-boundary.md) | Accepted | Headless Adapter Testing and Renderer Boundary |
| [ADR 0004](0004-resize-containment-and-lifecycle-boundary.md) | Accepted | Resize Containment and Lifecycle Boundary |
| [ADR 0005](0005-layout-engine-extension-boundary.md) | Accepted | Layout Engine Extension Boundary |
| [ADR 0006](0006-mind-map-layout-strategy.md) | Accepted | Mind Map Layout Strategy |
| [ADR 0007](0007-knowledge-canvas-foundations.md) | Accepted | Knowledge Canvas Foundations |
| [ADR 0008](0008-semantic-surface-and-framework-adapter-boundary.md) | Accepted | Semantic Surface and Framework Adapter Boundary |
| [ADR 0009](0009-node-kit-and-adapter-local-mapping-boundary.md) | Accepted | Node Kit and Adapter-Local Mapping Boundary |

Future records should use the next zero-padded number.
