...
use crate::rules::{
    DeleteDecision, Diagnostic, EdgeEndpoint, InsertNodeSpec, plan_connect,
    plan_connect_by_inserting_node, plan_connect_typed, plan_connect_with_mode, plan_delete_edge,
    plan_delete_elements_with_policy, plan_delete_node_with_policy, plan_reconnect_edge,
...
fn insert_edge(graph: &mut Graph, id: EdgeId, from: PortId, to: PortId) {
    graph.edges.insert(
        id,
        Edge {
            kind: EdgeKind::Data,
...
fn plan_delete_node_respects_policy_and_cascades_incident_edges() {
    ...
    assert!(matches!(
        &accepted.ops[0],
        jellyflow_core::ops::GraphOp::RemoveNode { id, edges, .. }
...
fn plan_delete_elements_dedupes_node_cascaded_edges() {
    ...
    assert!(
        plan.ops.iter().any(
            |op| matches!(op, jellyflow_core::ops::GraphOp::RemoveNode { id, .. } if *id == a)
        )
    );
    assert!(
        plan.ops.iter().any(
            |op| matches!(op, jellyflow_core::ops::GraphOp::RemoveNode { id, .. } if *id == b)
        )
    );
